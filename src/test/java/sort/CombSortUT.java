...
public class CombSortUT {

    @Test
    public void comb_sort_should_throw_arg_exception_when_list_is_null() {
        Assertions.assertThatThrownBy(() -> CombSort.sort(null))
                .isInstanceOf(ArgException.class)
                .hasMessage("List to sort is missing");
    }// comb_sort_should_throw_arg_exception_when_list_is_null()

    @Test
    public void comb_sort_should_throw_arg_exception_when_list_is_empty() {
        Assertions.assertThatThrownBy(() -> CombSort.sort(new ArrayList<Integer>()))
                .isInstanceOf(ArgException.class)
                .hasMessage("List to sort is missing");
    }// comb_sort_should_throw_arg_exception_when_list_is_empty()

    @Test
    public void comb_sort_should_be_equals_to_1() throws ArgException {
        List<Integer> list = new ArrayList<>();
        list.add(1);
        CombSort.sort(list);
        Assertions.assertThat(list.toString()).isEqualTo("[1]");
    }// comb_sort_should_be_equals_to_1()

    @Test
    public void comb_sort_should_be_equals_to_1_2() throws ArgException{
        List<Integer> list = new ArrayList<>();
        list.add(2);
        list.add(1);
        CombSort.sort(list);
        Assertions.assertThat(list.toString()).isEqualTo("[1, 2]");
    }// comb_sort_should_be_equals_to_1_2()
