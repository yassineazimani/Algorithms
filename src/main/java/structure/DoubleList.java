...
public class DoubleList<T> {
    ...
    public void add(T value) {
        if (this.head == null) {
            ...
            cursor.setNextLink(new DoubleLink(cursor.getIndex() + 1, value, cursor, null));
        }
    }// add()
    ...
    public void add(int idx, T value) throws IndexOutOfRangeException {
        ...
        for (; cursor.getNextLink() != null; cursor = cursor.getNextLink()) {
            if (cursor.getIndex() == idx) {
                addLinkAtIndex(cursor.getPrevLink(), cursor, idx, value);
                incrementIdx = true;
            }
            if(incrementIdx){
    ...
    private void addLinkAtIndex(DoubleLink<T> previousCursor, DoubleLink<T> currentCursor, int idx, T value){
        DoubleLink newLink = new DoubleLink(idx, value, previousCursor, currentCursor);
